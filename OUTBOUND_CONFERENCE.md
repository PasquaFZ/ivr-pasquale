...
- quede conectado con el cliente en cuanto alguien (o el buzón) atiende;
...
En las llamadas salientes no se reproduce ningún mensaje automático (ni saludo ni aviso
de grabación). El operador habla directamente con la persona o deja el mensaje en el
buzón.
...
Se inicia antes de crear la llamada al cliente, por lo que incluye la espera y la
conversación o mensaje.
...
5. Si la pata del cliente termina como `busy`, `failed`, `no-answer` o `canceled`,
   redirige al operador al aviso de no respuesta.
6. Si el operador abandona, intenta terminar la pata del cliente.
...
Si persona o buzón atiende:
  ...
  -> ambos quedan conectados sin mensaje automático
  -> operador oye a la persona o la locución/tono del buzón
...
- si responde una persona, el operador puede hablar de inmediato;
...
1. Persona atiende: ambos quedan conectados y conversan sin mensaje previo.
